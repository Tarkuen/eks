...
public class RestordreRepository implements RestordreInterface{
    ...
    public List<Restordre> readAllRestordrer() {
        ...
        try(Connection conn = databaseConnection.getConn()){
            ...
            ResultSet rs = stmt.executeQuery("SELECT restordre_nummer, restordrer.kunde_nummer, kundeimport.kunde_navn, restordrer.vare_nummer, vareimport.vare_navn, antal_varer, note, date_format(ekspeditionsDato, '%d,%M,%Y') AS ekspeditionsDato ,vare_leverance.leveranceNavn, vare_leverance.leverance_dato, aktiv   FROM eksamens_project.restordrer   left JOIN kundeimport on (restordrer.kunde_nummer = kundeimport.kunde_nummer)  left JOIN vareimport on restordrer.vare_nummer = vareimport.vare_nummer left JOIN vare_leverance on (vare_leverance.leveranceNavn = vareimport.vare_leverancenr);");
            while(rs.next()){
...
//                java.util.Date date = r.getEkspeditions_dato();
...
//
    ...
    public void create(Restordre restordre) {
        ...
        try(Connection conn = databaseConnection.getConn()){
            ...
            PreparedStatement pstmt = conn.prepareStatement("INSERT INTO eksamens_project.restordrer(kunde_nummer, vare_nummer, antal_varer,note,ekspeditionsDato) VALUES(?,?,?,?,?);");
            ...
            pstmt.setDate(5, restordre.getEkspeditions_dato());
    ...
    @Override
    public void updateEkspeditionsDato (Restordre restordre) {

        try(Connection conn = databaseConnection.getConn()){
            conn.setAutoCommit(false);
            PreparedStatement pstmt = conn.prepareStatement("UPDATE eksamens_project.restordrer SET ekspeditionsDato=? WHERE kunde_nummer=? AND vare_nummer=? AND antal_varer=? AND note=? AND aktiv=?;");
